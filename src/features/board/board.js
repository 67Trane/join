...
function renderHelper(section) {
  ...
  for (let i = 0; i < allTasks.length; i++) {
    ...
    let prio;
    if (allTasks[i].prio) {prio = getPrio(i, allTasks);} else {prio = "noprio.svg";}
    let checked = 0;
    let count;
    let subtaskslength = [];
    if (Object.keys(allTasks[i].subtask).length > 0) {
...
function getColors(i, allTasks) {
  const colors = normalizeStringList(allTasks[i].color);
  return colors.length ? colors : ["white"];
}
...
function getInitails(i, allTasks) {
  return normalizeStringList(allTasks[i].assignedto).map((name) => {
