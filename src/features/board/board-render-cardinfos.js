...
function renderEditCardInfos(iframeDocument, card) {
  ...
  let normalizedTask = normalizeTask(card);
  iframe.contentWindow.setEditTaskMeta({
  ...
  imgContainer.innerHTML += `
        <span class="d-none" id="deliver-names">${normalizedTask.assignedto.join(",")}</span>
  ...
  for (let i = 0; i < Object.values(normalizedTask.subtask).length; i++) {
    let keys = Object.keys(normalizedTask.subtask);
...
function getAssignedTo(card, iframeDocument) {
  const contacts = normalizeStringList(card.assignedto);
  const colors = normalizeStringList(card.color);

  for (let i = 0; i < contacts.length; i++) {
    let cleanInits = getContactInitialsFromName(contacts[i]);
    iframeDocument.getElementById("assigned-to").innerHTML += contactsHTML(contacts[i], cleanInits, colors[i] || "rgb(42, 54, 71)");
  }
}
